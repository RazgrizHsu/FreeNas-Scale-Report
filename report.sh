...
scrubAgeWarn=38         # Maximum age (in days) of last pool scrub before CRITICAL color will be used
...
backupLocation="/root/bak"   # Directory in which to save FreeNAS config backups


###### Auto-generated Parameters
host=$(hostname -s)
logfile="/tmp/nas_report"
subject="[nas] Report"
boundary="gc0p4Jq0M2Yt08jU534c0p"

if [ "$includeSSD" == "true" ]; then
    drives="$(ls -l "/sys/block" | grep -v 'devices/virtual' | sed -e 's:[[:blank:]]\{1,\}: :g' | cut -d ' ' -f "9" | sed -e 's:n[0-9]\{1,\}$::g' | uniq | sort -V)"
else
    drives="$(ls -l "/sys/block" | grep -v 'devices/virtual' | sed -e 's:[[:blank:]]\{1,\}: :g' | cut -d ' ' -f "9" | sed -e 's:n[0-9]\{1,\}$::g' | grep -v 'sd[a-z]\+ [0-9]\+G' | uniq | sort -V)"
fi
...
for pool in $pools; do
    ...
    if [ "$(echo "$statusOutput" | grep "scan" | awk '{print $2}')" = "scrub" ]; then
        ...
        scrubDate="$(echo "$statusOutput" | grep "scan" | \
awk '{month=$12; sub("Jan","01",month); sub("Feb","02",month); sub("Mar","03",month); sub("Apr","04",month); sub("May","05",month); sub("Jun","06",month); sub("Jul","07",month); sub("Aug","08",month); sub("Sep","09",month); sub("Oct","10",month); sub("Nov","11",month); sub("Dec","12",month); \
print $15"-"month"-"$13" "$14}')"
...
# Remove some un-needed junk from the output#!/bin/bash
...
# else
#     drives=$(for drive in $(sysctl -n kern.disks); do
#         if [ "$(smartctl -i /dev/"${drive}" | grep "SMART support is: Enabled")" ] && ! [ "$(smartctl -i /dev/"${drive}" | grep "Solid State Device")" ] && ! [ "$(smartctl -i /dev/"${drive}" | grep "QM000")" ]; then
#             printf "%s " "${drive}"
#         fi
#     done | awk '{for (i=NF; i!=0 ; i--) print $i }')
# fi
...
if [ "$includeSSD" == "true" ]; then
    # 使用新的命令來獲取所有的磁盤，包括 SSD
    drives="$(ls -l "/sys/block" | grep -v 'devices/virtual' | sed -e 's:[[:blank:]]\{1,\}: :g' | cut -d ' ' -f "9" | sed -e 's:n[0-9]\{1,\}$::g' | uniq | sort -V)"
else
    # 使用新的命令來獲取所有的磁盤，但排除 SSD（如果 'Solid State Device' 存在於描述中）
    drives="$(ls -l "/sys/block" | grep -v 'devices/virtual' | sed -e 's:[[:blank:]]\{1,\}: :g' | cut -d ' ' -f "9" | sed -e 's:n[0-9]\{1,\}$::g' | grep -v 'sd[a-z]\+ [0-9]\+G' | uniq | sort -V)"
fi
...
sed -i -e '/smartctl 7.4/d' "$logfile"
